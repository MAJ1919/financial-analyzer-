...
Modules:
  ...
  - derive_cash_flow_statement()  → Indirect-method CFS      (SRS §3.3)
...
def _build_key_to_label_map(statement: dict | None) -> dict[str, str]:
    """
    Build a mapping from canonical camelCase key → display label for rows
    in a stored FinancialStatement dict.

    This allows `_find_label_or_key()` to resolve a line item by its
    canonical key even when the display label differs across data sources.
    """
    if not statement or "rows" not in statement:
        return {}
...
def _find_label_or_key(
    ...
    """
    Resolve a line item to a label present in `df.index`.

    Strategy:
      1. Try each canonical *key* — look it up in `key_map` to get its
         actual display label, then check if that label exists in the
         DataFrame index.
      2. Fall back to direct label matching against the provided
         `labels` list.

    Returns the matched label string (suitable for `df.loc[]`) or None.
    """
    # 1. Key-based resolution (preferred — immune to label variations)
    for k in keys:
        label = key_map.get(k)
        if label and label in df.index:
            return label

    # 2. Label-based fallback (covers legacy data / Excel uploads)
    for lbl in labels:
        if lbl in df.index:
            return lbl
...
def derive_cash_flow_statement(
    income_statement: dict | None,
    balance_sheet: dict | None,
) -> dict:
    """
    Derive the indirect-method CFS from IS + BS data.
    Requires ≥ 2 years of BS data.
    ...
    Investing Activities:
        − Δ PP&E (proxy for CapEx, i.e. ending PP&E − beginning PP&E + D&A)

    Financing Activities:
        + Δ Total Debt
        − Dividends Paid (placeholder — not derivable from IS/BS alone)

    Net Change in Cash:
        = Operating + Investing + Financing
    """
    is_l, bs_l, _ = _build_lookups(income_statement, balance_sheet)
    bs_df = _to_dataframe(balance_sheet)
    is_df = _to_dataframe(income_statement)

    if bs_df.shape[1] < 2:
        return {
            "error": "Cash Flow Statement requires a minimum of 2 years of Balance Sheet data.",
            "years": [],
        }

    # Build key→label maps for key-based resolution
    bs_key_map = _build_key_to_label_map(balance_sheet)
    is_key_map = _build_key_to_label_map(income_statement)

    bs_years = list(bs_df.columns)
    cfs_years = bs_years[1:]  # Can only compute starting from year 2

    def bs_get(label: str, year: str) -> float:
        try:
            v = bs_df.loc[label, year]
            return float(v) if not pd.isna(v) else 0.0
        except KeyError:
            return 0.0

    def is_get(label: str, year: str) -> float:
        try:
            v = is_df.loc[label, year]
            return float(v) if not pd.isna(v) else 0.0
        except KeyError:
            return 0.0

    # ── Resolve labels for each line item ────────────────────────
    # Each call provides: (df, key_map, [canonical keys], [label variants])
    # Keys from manualEntryTemplate.json; labels include template labels + common alternatives.

    bs_cash_label = _find_label_or_key(
        bs_df, bs_key_map,
        keys=["cashAndEquivalents"],
        labels=["Cash & Cash Equivalents", "Cash, Cash Equivalents & STI",
                "Cash and Cash Equivalents", "Cash"],
    )
    bs_ar_label = _find_label_or_key(
        bs_df, bs_key_map,
        keys=["accountsReceivable"],
        labels=["Accounts Receivable, Net", "Accounts Receivable",
                "Accounts & Notes Receiv", "Trade Receivables", "Receivables"],
    )
    bs_inv_label = _find_label_or_key(
        bs_df, bs_key_map,
        keys=["inventory"],
        labels=["Inventories", "Inventory", "Raw Materials",
                "Finished Goods", "Stock"],
    )
    bs_ap_label = _find_label_or_key(
        bs_df, bs_key_map,
        keys=["accountsPayable"],
        labels=["Accounts Payable", "Payables & Accruals",
                "Trade Payables", "Creditors"],
    )
    bs_ppe_label = _find_label_or_key(
        bs_df, bs_key_map,
        keys=["propertyPlantEquipment", "ppeHeader"],
        labels=["Property, Plant & Equip", "Property, Plant & Equip, Net",
                "Property, Plant & Equipment", "PP&E", "Net PP&E"],
    )
    bs_ltd_label = _find_label_or_key(
        bs_df, bs_key_map,
        keys=["longTermDebt"],
        labels=["LT Borrowings", "Long-term Debt", "Long Term Debt",
                "LT Debt"],
    )
    bs_std_label = _find_label_or_key(
        bs_df, bs_key_map,
        keys=["currentDebt", "stBorrowings"],
        labels=["Current Portion of LT Debt", "ST Borrowings",
                "Short-term Debt", "Short Term Debt"],
    )
    bs_tl_label = _find_label_or_key(
        bs_df, bs_key_map,
        keys=["totalLiabilities"],
        labels=["Total Liabilities"],
    )

    is_ni_label = _find_label_or_key(
        is_df, is_key_map,
        keys=["netIncome"],
        labels=["Net Income, GAAP", "Net Income", "Net Profit",
                "Profit for the Year", "Net Earnings",
                "Net Income Avail to Common, GAAP"],
    )
    is_da_label = _find_label_or_key(
        is_df, is_key_map,
        keys=["depreciation", "depreciationCogs"],
        labels=["Depreciation & Amortization", "Depreciation & Amortization (D&A)",
                "D&A", "Depreciation"],
    )

    operating: dict[str, dict] = {}
    investing: dict[str, dict] = {}
    financing: dict[str, dict] = {}
    net_change: dict[str, Any] = {}

    for year, prev_year in zip(cfs_years, bs_years):
        try:
            if int(year) - int(prev_year) != 1:
                continue
        except ValueError:
            pass  # If not an integer (e.g., FY21), we can't reliably detect gaps, so proceed
            
        # ── Operating Activities ─────────────────────────────
        net_income = is_get(is_ni_label, year) if is_ni_label else 0.0
        da = abs(is_get(is_da_label, year)) if is_da_label else 0.0

        # Safely handle missing prior year data by defaulting to 0
        current_ar = bs_get(bs_ar_label, year) if bs_ar_label else 0.0
        prior_ar = bs_get(bs_ar_label, prev_year) if bs_ar_label else 0.0
        
        current_inv = bs_get(bs_inv_label, year) if bs_inv_label else 0.0
        prior_inv = bs_get(bs_inv_label, prev_year) if bs_inv_label else 0.0
        
        current_ap = bs_get(bs_ap_label, year) if bs_ap_label else 0.0
        prior_ap = bs_get(bs_ap_label, prev_year) if bs_ap_label else 0.0

        # Operating Cash Flow = Net Income + Depreciation + (Prior Year AR - Current Year AR) + (Prior Year Inventory - Current Year Inventory) + (Current Year AP - Prior Year AP)
        delta_ar = prior_ar - current_ar
        delta_inv = prior_inv - current_inv
        delta_ap = current_ap - prior_ap

        operating_cf = round(net_income + da + delta_ar + delta_inv + delta_ap, 2)

        operating[year] = {
            "Net Income":                    round(net_income, 2),
            "Depreciation & Amortization":   round(da, 2),
            "Change in Accounts Receivable": round(delta_ar, 2),
            "Change in Inventory":           round(delta_inv, 2),
            "Change in Accounts Payable":    round(delta_ap, 2),
            "Total Operating Cash Flow":     operating_cf,
        }

        # ── Investing Activities ─────────────────────────────
        # CapEx proxy: change in PP&E + D&A (ending PPE - beginning PPE + DA)
        capex_proxy = 0.0
        if bs_ppe_label:
            capex_proxy = -(bs_get(bs_ppe_label, year) - bs_get(bs_ppe_label, prev_year) + da)

        investing_cf = round(capex_proxy, 2)
        investing[year] = {
            "Capital Expenditures (estimated)": round(capex_proxy, 2),
            "Total Investing Cash Flow":         investing_cf,
        }

        # ── Financing Activities ─────────────────────────────
        delta_ltd = 0.0
        if bs_ltd_label:
            delta_ltd = bs_get(bs_ltd_label, year) - bs_get(bs_ltd_label, prev_year)

        delta_std = 0.0
        if bs_std_label:
            delta_std = bs_get(bs_std_label, year) - bs_get(bs_std_label, prev_year)

        delta_debt = delta_ltd + delta_std

        financing_cf = round(delta_debt, 2)
        financing[year] = {
            "Net Change in Debt":       round(delta_debt, 2),
            "Total Financing Cash Flow": financing_cf,
        }
        ...
        net_change[year] = {
            ...
            "Beginning Cash":     round(bs_get(bs_cash_label, prev_year), 2) if bs_cash_label else None,
            "Ending Cash":        round(bs_get(bs_cash_label, year), 2) if bs_cash_label else None,
        }
