...
`backend/app/models/manualEntryTemplate.json`, loaded by `app/models/statement_templates.py`, served at `GET /api/templates/statements`. The frontend fetches it (cached in `projectStore.js`) to initialize manual entry; the Excel parser matches upload rows against its labels **strictly**. There is no frontend copy anymore — line-item changes happen in that one JSON. `frontend/public/Saudi_Template.xlsx` (the file users download, fill in and re-upload) must be regenerated to match: `python scripts/generate_templates.py` from `backend/`. That script now builds the workbook **from the canonical JSON** — it used to parse an out-of-repo markdown file, which is how the download drifted (`Operating Income` vs the canonical `Operating Income (EBIT)`) and made the template's own rows come back as `unmapped_rows`. `tests/test_template_workbook.py` fails on any drift, including a round-trip assert that uploading the shipped template yields zero unmapped rows.

**One curated exception to strict matching:** `LEGACY_LABEL_ALIASES` in `excel_parser.py` maps labels *this project previously shipped* onto their current canonical label (today: `Operating Income` → `Operating Income (EBIT)`). It exists because re-downloading a blank template is not a remedy for a user who has already entered data. It is **not** fuzzy matching — add an entry only for a label the repo actually published, and a test asserts every alias target is a live canonical label and every alias key is not. Unrecognised labels are still reported, never guessed. Distinct from `KEY_COMPAT_MAP` (`shared_utils.py`), which remaps canonical *keys* for the engines; this map is upload-time *labels*.

The upload response also carries `unmapped_hints` (`{sheet: {label: hint}}`) — a `difflib` nearest-canonical-label suggestion, or a note that the row was a duplicate — so the warning banner tells the user what to change. Duplicate labels in one sheet keep the first occurrence and report the rest instead of silently overwriting.
